...
func NewConPtyProcess(path string) *ConPtyProcess {
	return &ConPtyProcess{
		path: path,
	}
}

func (m *ConPtyProcess) Start(logCallback func(log string), args []string) error {
	// 如果已有实例在运行，先停止
	...
	// 创建一个新的底层管理器
	var err error
	if m.manager, err = BaseProcess.NewConPtyProcessManager(m.path); err != nil {
		return err
	}
...
func (m *ConPtyProcess) Stop() error {
	if m.manager == nil || !m.manager.IsRunning() {
...
func (m *ConPtyProcess) SendCommand(command string) error {
	if m.manager == nil || !m.manager.IsRunning() {
...
func (m *ConPtyProcess) IsRunning() bool {
	if m.manager == nil {
		return false
	}
...
func (m *ConPtyProcess) GetStatus() (entity.ProcessState, error) {
	if m.manager == nil || !m.manager.IsRunning() {
