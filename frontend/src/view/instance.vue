    ...
    <div class="section-content fade-in-up delay-1">
      <div class="instances-grid">
        <instanceItem
            ...
            :key="instance.instanceInfo.name"
            :name="instance.instanceInfo.name"
            @onShowChild="showDataWindow"
            @opened="runningServers.push({name: instance.instanceInfo.name})"
            @closed="removeFromRunningServers(instance.instanceInfo.name)"
        />

        <div v-if="instancesStore.instances.length === 0" class="empty-state">
...
const showDataWindow = (name: string) => {
  if (childWindowData.value.some(item => item.name === name)) {
    ElMessage({
      message: "当前实例窗口已打开",
      type: "warning",
    });
...
const dataWindowClosed = (name: string) => {
  childWindowData.value = childWindowData.value.filter(item => item.name !== name)
}

const createInstance = () => {
  instancesStore.createInstance(newInstance.value)
...
onMounted(() => {
  instancesStore.initializeStore()
})
</script>
