...
function handleBrowse(item: PluginSettingItem) {
  ...
  if (item.type === "select_dir") {
    ...
    //     // })
    //   } else {
    //     appConfigStore.appConfig.SetValueOfKey(item.key, path).then(() => {
    //       item.value = path;
    //       ElMessage({
    //         message: '设置成功',
    //         type: 'success',
  ...
  pluginListStore.loadingPromise.then(() => {
    for (const key of pluginListStore.pluginList.keys()) {
      ...
      if (!plateFound) {
        plugins.value.push({
          plate: setting.plate,
          items: items as PluginSetting['items']
        });
      }
    }
  })

  appConfigStore.appConfig.GetAllValue().then((conf: any) => {
    for (const plugin of plugins.value) {
      for (const item of plugin.items) {
        ...
        watch(item, () => {
              ...
              appConfigStore.appConfig.SetValueOfKey(item.key, item.value).then(() => {
                ElMessage({
                  message: '设置成功',
                  type: 'success',
                })
